...
import { CreateFoodLogRequest, UpdateFoodLogRequest, FoodLogSearchParams, DailyNutritionSummary, CreateBulkFoodLogRequest, BulkFoodLogResponse } from '../models/FoodLog';
...
export class FoodLogService {
  ...
  private async updateFoodLogQuantity(
    ...
    try {
      ...
      throw new Error(`Failed to update food log quantity: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  public async createFoodLog(data: CreateFoodLogRequest): Promise<IFoodLog> {
    try {
      ...
      const existingLog = await this.findExistingFoodLog(data.user, data.meal, logDate, data.mealType);
      ...
      if (existingLog) {
        ...
        await savedFoodLog.populate('meal', 'name calories protein fat carbs servingSize emoji');
  ...
  public async createBulkFoodLog(data: CreateBulkFoodLogRequest): Promise<BulkFoodLogResponse> {
    try {
      // Validate input
      ...
      if (itemsToUpdate.length > 0) {
        const updatePromises = itemsToUpdate.map(async ({ item, existingLog }) => {
          try {
            ...
            if (!updatedLog) {
              throw new Error(`Failed to update food log for meal ${item.meal}`);
            }

            return updatedLog;
          } catch (error) {
            throw new Error(`Failed to update food log for meal ${item.meal}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          }
        });

        const updateResults = await Promise.all(updatePromises);
        const validUpdates = updateResults.filter(log => log !== null) as IFoodLog[];
  ...
  public async updateFoodLog(id: string, data: UpdateFoodLogRequest): Promise<IFoodLog | null> {
    try {
  ...
  public async getFoodLogsByUser(
    ...
    try {
      ...
      throw new Error(`Failed to fetch food logs for user ${userId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  public async searchFoodLogs(params: FoodLogSearchParams): Promise<{ data: IFoodLog[], total: number, page: number, pages: number }> {
    const { 
      userId,
      mealType,
      startDate,
  ...
  public async getDailyNutritionSummary(userId: string, date: number): Promise<DailyNutritionSummary> {
    try {
      const summary = await (FoodLog as any).getDailyNutritionSummary(userId, date);
      return summary;
    } catch (error) {
      if (error instanceof Error) {
  ...
  public async getNutritionSummaryRange(
    ...
    try {
      const summaries = await (FoodLog as any).getNutritionSummaryRange(userId, startDate, endDate);
      return summaries;
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(`Failed to get nutrition summary range: ${error.message}`);
      }
    ...
    try {
      const stats = await FoodLog.aggregate([
        { $match: { user: userId } },
        {
          $group: {
            _id: null,
            totalLogs: { $sum: 1 },
            totalCalories: { $sum: { $multiply: ['$quantity', '$meal.calories'] } },
            totalProtein: { $sum: { $multiply: ['$quantity', '$meal.protein'] } },
      ...
      result.mealTypeBreakdown.forEach((item: any) => {
        if (mealTypeBreakdown.hasOwnProperty(item.mealType)) {
          mealTypeBreakdown[item.mealType as keyof typeof mealTypeBreakdown] += item.quantity;
        }
      });
  ...
  public async getMonthlyNutritionTrend(
    ...
    try {
      ...
      return await this.getNutritionSummaryRange(userId, startDateEpoch, endDate);
    } catch (error) {
