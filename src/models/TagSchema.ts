...
interface ITagModel extends mongoose.Model<ITag> {
  ...
  findByCategory(category: string, page?: number, limit?: number): Promise<ITag[]>;
...
tagSchema.statics.searchByName = function(query: string, page: number = 1, limit: number = 10) {
...
tagSchema.statics.findByCategory = function(category: string, page: number = 1, limit: number = 10) {
  const skip = (page - 1) * limit;
  return this.find({ 
...
tagSchema.statics.findActiveTags = function(page: number = 1, limit: number = 10) {
  const skip = (page - 1) * limit;
  return this.find({ 
    isActive: true 
  })
